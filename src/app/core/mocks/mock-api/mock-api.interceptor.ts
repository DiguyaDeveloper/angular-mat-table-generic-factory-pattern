...
import { CeccoffMockApiService } from "./mock-api.service";

@Injectable({
  providedIn: "root",
})
export class CeccoffMockApiInterceptor implements HttpInterceptor {
  /**
   * Constructor
   */
  constructor(
    @Inject(CECCOFF_MOCK_API_DEFAULT_DELAY) private _defaultDelay: number,
    private _ceccoffMockApiService: CeccoffMockApiService
  ) {}
  ...
  intercept(
    ...
    const { handler, urlParams } = this._ceccoffMockApiService.findHandler(
